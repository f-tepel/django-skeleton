...
class UserDetailTestCase(TestCase):
  ...
  def test_get_user(self):
    ...
    res = self.client.get(f'{self.ROUTE}{self.db.customer_user.id}')
    utils.get_detail_check(self, res)

  def test_get_other_user(self):
    self.client.login(username=self.db.CUSTOMER_EMAIL, password=self.db.PASSWORD)
    res = self.client.get(f'{self.ROUTE}{self.db.customer_user_2.id}')
  ...
  def test_update_user(self):
    self.client.login(username=self.db.CUSTOMER_EMAIL, password=self.db.PASSWORD)
    res = self.client.put(f'{self.ROUTE}{self.db.customer_user.id}', data={
  ...
  def test_update_another_user(self):
    self.client.login(username=self.db.CUSTOMER_EMAIL, password=self.db.PASSWORD)
    res = self.client.put(f'{self.ROUTE}{self.db.customer_user_2.id}', data={
  ...
  def test_delete_another_user(self):
    self.client.login(username=self.db.CUSTOMER_EMAIL, password=self.db.PASSWORD)
    res = self.client.delete(f'{self.ROUTE}{self.db.customer_user_2.id}')

    utils.forbidden_check(self, res)

  def test_delete_unauthenticated(self):
    res = self.client.delete(f'{self.ROUTE}{self.db.customer_user.id}')
